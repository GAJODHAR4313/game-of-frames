...
const Showreel = () => {
  ...
  return (
    // Background fixo no preto (#050505)
    <section id="showreel" ref={containerRef} className="relative bg-[#050505] py-16 md:py-24 lg:py-40 border-t border-white/5 overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-5 sm:px-8 md:px-10 lg:px-20 relative z-10">
        ...
        <div className="mb-12 md:mb-20">
          <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-4">
            ...
            <h2 className="text-amber-500 uppercase tracking-[0.4em] md:tracking-[0.6em] text-[9px] md:text-[10px] font-black">Edits</h2>
          </div>
          
          <h3 className="text-5xl sm:text-6xl md:text-8xl font-black text-white uppercase tracking-tighter leading-none">
            Selected <br />
            <span className="italic text-white/20">Moments.</span>
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {reels.map((reel) => (
            <div 
              key={reel.id} 
              // Card fixo no neutral-900
              className="reel-card relative aspect-[9/16] group overflow-hidden bg-neutral-900 border border-white/5 rounded-sm shadow-2xl"
            >
              <video 
                src={reel.videoSrc}
              ...
              <div className="absolute inset-0 pointer-events-none border-[1px] border-transparent group-hover:border-amber-500/30 transition-colors duration-500">
                <div className="absolute top-4 left-4 w-4 h-4 border-t-2 border-l-2 border-amber-500 opacity-0 group-hover:opacity-100 transition-all duration-500"></div>
                <div className="absolute bottom-4 right-4 w-4 h-4 border-b-2 border-r-2 border-amber-500 opacity-0 group-hover:opacity-100 transition-all duration-500"></div>
              </div>
        ...
        <div className="mt-12 md:mt-16 flex justify-center md:justify-end">
            ...
            >
                <span className="text-white text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em]">Load More Clips</span>
                <span className="w-8 md:w-12 h-[1px] bg-white group-hover:w-16 md:group-hover:w-20 group-hover:bg-amber-500 transition-all duration-500"></span>
            </a>
        </div>
      </div>
    </section>
